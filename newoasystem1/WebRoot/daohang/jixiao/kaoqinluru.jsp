...
.sidebar-nav {
 ...
 <!-- topbar starts -->
					...
					<div class="row-fluid">
						<div class="box span12">
							<div class="box-header well">
								...
								<div class="box-icon">
					          <input type=button onclick="window.open('bukaoqin.jsp')" value="更新完毕"><br>
						</div>
							</div>
  	  ...
  	  <% 
   if(request.getSession().getAttribute("quanxian").equals("superman")){
   ArrayList<String> id,zuren=new ArrayList<String>();
   zuren=(ArrayList<String>)request.getSession().getAttribute("zuren");
   id=(ArrayList<String>)request.getSession().getAttribute("id");
   for(int i=0;i<(zuren.size()/15+1);i++){
   %>
   <tr>
   <% 
   for(int j=0;j<15;j++){
   ...
   if(a<zuren.size()){
   %>
   <td class="center"><a href="kaoqinkoukuan.jsp?name=<%=zuren.get(a)%>&userid=<%=id.get(a)%>"><%=zuren.get(a)%></a></td>
   <% 
   }else{
    %>
   <td class="center"></td>
